 ...
 */
struct nodeStruct* List_createNode(int size,int index)
{
 ...
 */
struct nodeStruct* List_findNode1(struct nodeStruct *head, int index)
{
	struct nodeStruct *current = head;
	while (current != NULL) {
...
struct nodeStruct* List_findNode2(struct nodeStruct *head, int index)
{
	struct nodeStruct *current = head;
	while (current != NULL) {
		int temp = current->size + current->index;
		if (temp == index) {
			return current;
		}
		current = current->next;
